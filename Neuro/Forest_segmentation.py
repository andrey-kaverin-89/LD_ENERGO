...
def train(dataset_path = DATASET_PATH, image_dir = IMAGE_DIR, mask_dir = MASK_DIR):
    ...
    for i in range(n_epoches):
        ...
        for x_val, y_val in val_dataloader:
            ...
            mae_val = loss(y_pred, y_val)

            hist.append(mae_val)
        mae_loss = sum(hist)/len(hist)
    
    torch.save(model.state_dict(), './chp/model_forest.pt') 

    
## Инференс модели       
def inference(x_path, save_path = './inference', model_path = './chp/model_forest.pt'):
    model = torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet',
        in_channels=3, out_channels=1, init_features=32, pretrained=False)
    ...
    pred_mask = pred_mask.detach().numpy()*256
    ...
    cv2.imwrite(save_path+ '/' + x_path.split('/')[-1] + '_segmentation_forest.bmp', pred_mask)
